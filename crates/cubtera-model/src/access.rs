//! Unit access policy (P7 v2->v3 port of `cubtera_domain::access`).
//!
//! A pure function returning [`AccessDecision`]; the caller turns a
//! `Denied` decision into an `AppError`, and the CLI/API/server decide
//! what that means (message + exit code, or an HTTP status).
//!
//! `affinityTags` is a universal-quantification check ("every resolved
//! dimension must overlap the unit's tags") - a different shape than
//! [`crate::policy::Policy`]'s single-record match, so it stays a
//! dedicated check here rather than being folded into the `Selector`-based
//! engine (see `policy.rs`'s module doc comment).
...
impl AccessPolicy {
    ...
    /// dimensions *and their ancestors* (i.e. every `Dimension::key_path`
    /// entry, flattened) - `allowList`/`denyList` can reference a parent
    /// dimension to gate an entire subtree.
    pub fn evaluate(
        manifest: &Manifest,
