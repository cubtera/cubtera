//! Unit manifest types (P7 v2->v3 port of `cubtera_domain::manifest`).
//!
//! A manifest defines how a unit should be executed. The TOML schema
//! mirrors v1's `manifest.toml` as-is - like the inventory naming
//! convention, this is a stable per-unit config format, not something the
//! rewrite gets to redesign.
...
pub struct Manifest {
    ...
    /// overlap with this list (see [`crate::access::AccessPolicy`])
    #[serde(alias = "affinity_tags")]
    pub affinity_tags: Option<Vec<String>>,
    /// Runner type: "tf"/"terraform", "tofu"/"opentofu", "bash"/"sh", "helm"
    #[serde(rename = "type")]
    ...
    /// `inlet_command`, `outlet_command` (per-runner-type meaning)
    ...
    /// `cubtera_model::state_projection::project_state_key` for how
    /// `dims`/`ext` are resolved when left unset). Empty by default - most
    /// units consume nothing.
    #[serde(default)]
...
impl Manifest {
    ...
    pub fn from_toml(content: &str) -> Result<Manifest, ModelError> {
        toml::from_str(content).map_err(|e| ModelError::InvalidManifest(e.to_string()))
    }
...
/// required dimension types - see
/// `cubtera_model::state_projection::project_state_key`.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct InputSpec {
    ...
    /// `OutputSet::schema_version` must satisfy - a mismatch is a hard
    /// `plan`/`apply` failure, never a silent stale read. `None` skips the
    /// check entirely.
    #[serde(default)]
