...
//!
//! v1 wrote directly to disk from inside "pure" domain methods
//! (`Unit::copy_files_to_temp`, `Unit::write_dimension_data`,
//! `Unit::remove_temp_folder`) - I/O buried in a layer that's supposed to be
//! testable without a filesystem. Here the domain only *describes* the work
//! as a [`MaterializationPlan`]: an ordered list of [`MaterializationStep`]s
//! with all content already computed. An adapter behind a `Workspace` port
//! executes it; `--dry-run` just prints it. Directory copies stay
//! directory-granular (`CopyDir`) rather than being expanded file-by-file,
//! since enumerating a directory's contents is itself I/O the domain has no
//! business doing - the adapter walks the tree at apply time.

use std::fmt;
use std::path::PathBuf;

/// A single filesystem action needed to materialize a unit's temp working
/// directory. Every field is fully resolved data - no further business logic
/// runs at apply time, only I/O.
