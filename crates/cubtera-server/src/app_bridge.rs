...
//!
//! Copy of `crates/cubtera/src/app_bridge.rs` - `cubtera-app` cannot
//! depend on `cubtera-core` (see the crate table in
//! docs/specs/2026-09-03-cubtera-v3-architecture.md section 3), and this bridge
//! itself cannot live in a shared crate either without creating a
//! dependency edge from `cubtera-app` to something that (transitively)
//! depends on `cubtera-core`. Small, deliberate duplication that goes
//! away along with both bridges once v2's `cubtera-core`/
//! `cubtera-persistence` are retired.
